...
const App: React.FC = () => {
  const [publicModuleIds, setPublicModuleIds] = React.useState<string[]>([]);

  React.useEffect(() => {
    const API_URL = getApiUrl();
    fetch(`${API_URL}/api/auth/public-modules`)
      .then(res => res.json())
      .then(data => {
        if (data.success) setPublicModuleIds(data.publicIds);
      })
      .catch(() => { /* fallback: tất cả đều cần mã */ });
  }, []);
    ...
    <AppShell>
      <ErrorBoundaryWithReset>
        <Suspense fallback={<LoadingFallback />}>
          <Routes>
            <Route path="/" element={<Dashboard publicModuleIds={publicModuleIds} />} />
            <Route path="/ntp-admin-2026" element={<AdminDashboard />} />
              ...
              <Route
                ...
                element={
                  <AccessGuard 
                    moduleId={app.id} 
                    moduleName={app.name}
                    isPublic={publicModuleIds.includes(app.id)}
                  >
                    <app.component />
                  </AccessGuard>
                }
