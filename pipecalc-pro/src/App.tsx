...
// Surcharge pressures Ps (kN/m2) from Table A.1
const getTrafficLoad = (traffic: string, H: number) => {
  if (traffic === 'None') return 0;
  
  // Simplified interpolation based on Table A.1
  if (traffic === 'Main') {
    if (H <= 0.6) return 120;
    if (H <= 1.0) return 74;
...
export default function App() {
  ...
  // Update E when material changes, and ensure material is valid for systemType
  useEffect(() => {
    setParams(p => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setParams(p => {
      ...
      if (name === 'corrugatedMaterial') {
        newParams.snType = value === 'HDPE' ? 'SN8' : 'SN8';
        newParams.snValue = 8;
  ...
  const results = useMemo(() => {
    ...
    // Deflection (Spangler) with internal pressure effect (rerounding)
    const Dl = 1.5;
    ...
    // Pcr in MPa then convert to kPa
    ...
    // Combined Stress Analysis (BS EN 1295-1 / Method B principles)
    // 1. Hoop Stress from Internal Pressure (MPa)
    const hoopStressPi = (Pi_MPa * Dm) / (2 * actual_e);
    
    // 2. Bending Stress from Deflection (MPa)
    // Formula: sigma_b = 3 * E * e * delta_x / Dm^2 (approximate for flexible pipes)
    const bendingStress = (3 * E * actual_e * delta_x) / Math.pow(Dm, 2);
    
    // 3. Combined Stress (MPa)
    const combinedStress = hoopStressPi + bendingStress;
    const isStressPass = MATERIALS[params.material].sigma_d ? combinedStress <= MATERIALS[params.material].sigma_d! : true;
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 leading-tight tracking-tight">PipeCalc Pro</h1>
              <p className="text-xs text-slate-500 font-medium">Tính toán chôn lấp ống theo BS EN 1295-1</p>
            </div>
          </div>
          <button 
              ...
              <div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left border-collapse">
                    ...
                    <tbody>
                      <tr>
                        <td className="border p-2 font-medium">Main roads</td>
                        ...
                        <td className="border p-2">9.92</td>
                      </tr>
                      <tr>
                        <td className="border p-2 font-medium">Light roads</td>
                        <td className="border p-2">-</td>
                        <td className="border p-2">120</td>
                        <td className="border p-2">63.2</td>
                      ...
                      <tr>
                        ...
                        <td className="border p-2">69.5</td>
                        <td className="border p-2">36.1</td>
                        <td className="border p-2">12.3</td>
                        <td className="border p-2">3.43</td>
                        <td className="border p-2">1.56</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {['HDPE (PE 100)', 'PVC-U'].includes(params.material) && (
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      <SelectField
                        label="Phương pháp nhập chiều dày"
                        name="thicknessInputType"
                        value={params.thicknessInputType}
                        onChange={handleInputChange}
                        options={[
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <SelectField
                        label="Góc ôm của lớp lót (Bedding Angle)"
                        name="beddingAngle"
                        value={params.beddingAngle}
                        onChange={handleInputChange}
                        options={[
                          { label: '0°', value: '0' },
                          { label: '60°', value: '60' },
                      ...
                      <div className="p-3 bg-blue-50/50 border border-blue-100 rounded-lg">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-sm font-mono font-bold text-blue-600">{BEDDING_CONSTANTS[params.beddingAngle]}</span>
                        </div>
                        <p className="text-[10px] text-blue-800 leading-tight italic">
                          {BEDDING_DESCRIPTIONS[params.beddingAngle]}
                        </p>
                      </div>
                    </div>
                </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="space-y-4">
                  <SelectField
                    label="Tải trọng giao thông (Table A.1)"
                    name="traffic"
          ...
          <div className="lg:col-span-4">
            <div className="bg-slate-900 rounded-2xl shadow-xl text-white sticky top-24 overflow-hidden border border-slate-800">
